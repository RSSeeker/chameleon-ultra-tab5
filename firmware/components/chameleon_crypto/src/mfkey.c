...
//-----------------------------------------------------------------------------
//
// Tab5 port note: this file is a VERBATIM copy of software/src/mfkey.c from the
// ChameleonUltra repository, so that the darkside key-recovery core is byte for
// byte the same algorithm the official CLI drives via darkside.exe. The only
// thing that differs is where lfsr_common_prefix() gets its memory from, and
// that lives in crapto1.c (CP_MALLOC -> PSRAM).
//
// Do not "improve" this file: tools/probe_darkside.py compiles it side by side
// with the upstream original and compares outputs for exactly that reason.
#include "mfkey.h"
#include "crapto1.h"
